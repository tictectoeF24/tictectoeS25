...
// ─── MERGE NOTE ───────────────────────────────────────────────────────────────
// dev branch: added forwardRef + useImperativeHandle, onOpenChange prop,
//             textRef / uiRef, and explicit animateChat calls.
// note branch: had a separate useEffect calling animateChat(isOpen) — removed
//              in favour of the dev approach (explicit calls in toggleChat and
//              the visibility effect) to avoid double-firing animations.
// Bug fixed:  dev's close-button TouchableOpacity had a stray onOpenChange prop
//             referencing undefined setIsChatbotOpen — removed.
// ─────────────────────────────────────────────────────────────────────────────

const FloatingChatbot = forwardRef(
    (
        {
            context,
            pdfUrl,
        ...
        const textRef = useRef(null);   // dev: ref for TextInput
        const uiRef = useRef(null);     // dev: ref for floating button
        ...
        // Load chat history when paperId becomes available or chat opens
        useEffect(() => {
        ...
        // dev: animateChat is called explicitly here instead of a separate effect
        useEffect(() => {
        ...
        const loadChatHistory = async () => {
            try {
                ...
                if (chatHistory.length > 0) {
                    ...
                    console.log(
                        `Loaded ${formattedMessages.length} private messages from ${chatHistory.length} chat exchanges`
                    );
                    setMessages(formattedMessages);
                    setShowWelcomeMessage(false);
                    setAvailableQuestions(allQuestionSuggestions);
                } else {
            ...
            try {
                ...
                const requestData = {
                    ...
                    username:
                        ...
                        `User-${userInfo?.userId?.slice(-8)}` ||
                        "Anonymous User",
                };

                console.log("FloatingChatbot - Request data:", requestData);
                await axios.post(
                    `${BASE_URL_IN_CONFIG}/api/conversations/save-exchange`,
                    requestData
                );
        ...
        const toggleChat = () => {
            ...
            // dev: notify parent of open/close state
        ...
        const handleSend = async () => {
            ...
            setMessages((prev) => [
                ...prev,
                {
                    ...
                    from: "user",
                    username:
                        userInfo?.username || userInfo?.email?.split("@")[0] || "You",
            ...
            while (retryCount < maxRetries) {
                try {
                    const chatContext =
                        pdfContext || context || "General academic paper discussion";

                    console.log(
                        `Attempt ${retryCount + 1}: Sending request to:`,
                        `${BASE_URL_IN_CONFIG}/api/chatbot`
                    );
                    console.log("Request payload:", {
                        question: userMessage,
                        context: chatContext
                            ? chatContext.substring(0, 100) + "..."
                            : "No context",
                    });
                    ...
                    const aiResponse =
                        res.data.answer || "Sorry, I couldn't generate a response.";
                    ...
                    if (is503Error) {
                        errorMessage =
                            "🚦 The AI service is currently overloaded. This is a temporary issue on Google's end. Please try again in a few minutes, or try asking a simpler question.";
        ...
        return (
            ...
            >
                {!isOpen && !isEmbedded ? (
                    // Floating button — dev: added ref={uiRef}
                    <TouchableOpacity
                        ref={uiRef}
                        style={{
                            width: 56,
                    ...
                    >
                        ...
                        {!isEmbedded && (
                            ...
                            >
                                ...
                                {/* dev bug fixed: removed stray onOpenChange referencing undefined setIsChatbotOpen */}
                        ...
                        >
                            ...
                            <TouchableOpacity
                                ...
                                style={{
                                    marginLeft: 8,
                                    backgroundColor:
                                        loading || fetchingPdf || loadingHistory
                                            ? "#aaa"
                                            : "#064E41",
                                    padding: 10,
                                    borderRadius: 20,
                                ...
                                disabled={
                                    loading || fetchingPdf || loadingHistory || !input.trim()
                                }
                            >
